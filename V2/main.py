...
p = 20
times = 1
update_when_iter = False
table = np.zeros((2, 11))
for i in range(times):
    x_max = 100*np.ones(d)
    x_min = -100*np.ones(d)
    optimizer = WOA(fit_func=Sphere, update_when_iter=update_when_iter, 
    ...
    x_max = 10*np.ones(d)
    x_min = -10*np.ones(d)
    optimizer = WOA(fit_func=Schwefel_P222, update_when_iter=update_when_iter,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 100*np.ones(d)
    x_min = -100*np.ones(d)
    optimizer = WOA(fit_func=Quadric, update_when_iter=update_when_iter,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 10*np.ones(d)
    x_min = -10*np.ones(d)
    optimizer = WOA(fit_func=Rosenbrock, update_when_iter=update_when_iter,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 100*np.ones(d)
    x_min = -100*np.ones(d)
    optimizer = WOA(fit_func=Step, update_when_iter=update_when_iter,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    optimizer = WOA(fit_func=Quadric_Noise, update_when_iter=update_when_iter,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 500*np.ones(d)
    x_min = -500*np.ones(d)
    optimizer = WOA(fit_func=Schwefel, update_when_iter=update_when_iter,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    optimizer = WOA(fit_func=Rastrigin, update_when_iter=update_when_iter,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 5.12*np.ones(d)
    x_min = -5.12*np.ones(d)
    optimizer = WOA(fit_func=Noncontinuous_Rastrigin, update_when_iter=update_when_iter,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 32*np.ones(d)
    x_min = -32*np.ones(d)
    optimizer = WOA(fit_func=Ackley, num_dim=d, update_when_iter=update_when_iter,
                    num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 600*np.ones(d)
    x_min = -600*np.ones(d)
    optimizer = WOA(fit_func=Griewank, update_when_iter=update_when_iter,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
